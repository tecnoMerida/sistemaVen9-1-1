...
$pdf->SetFont('Arial','B',12);
$pdf->Cell(100,7,utf8_decode(''.strtoupper($reg03['grado_instruccion']).' '.strtoupper($reg['p_apellido']).' '.strtoupper($reg['s_apelllido']).', '.strtoupper($reg['p_nombre']).' '.strtoupper($reg['s_nombre']).''),0,0,'C');
$pdf->SetFont('Arial','',12);
$pdf ->Cell(70,7,utf8_decode('titular de la cédula de identidad N°'),0,1);
$pdf->SetX(20);
$pdf->SetFont('Arial','',12);
...
if(($cargos != 1 ) AND ($cargos != 3)) {
		...
		$pdf ->Cell(65,7,utf8_decode(''.strtoupper($reg04['nombre_cargo']).' de Telecomunicaciones'),0,0,'C');
	}else {
		$pdf ->Cell(65,7,utf8_decode(''.strtoupper($reg04['nombre_cargo']).' '),0,0,'C');
		}


if(($cargos != 1 ) AND ($cargos != 3)) {
		$pdf->SetFont('Arial','B',12);
		$pdf ->Cell(40,7,utf8_decode(''.strtoupper($reg05['nombre_oganismos']).''),0,1,'C');
		$pdf->SetX(20);
		$pdf->SetFont('Arial','',12);
		$pdf ->Cell(170,7,utf8_decode('y el Sistema de Atención de Emergencias 911., en un horario de 24 horas por'),0,1);
		$pdf->SetX(20);
		$pdf ->Cell(35,7,utf8_decode('72 horas libres.'),0,1);		
	}else {
		$pdf ->Cell(32,7,utf8_decode('del  Sistema  de '),0,1);
		$pdf->SetX(20);
		$pdf ->Cell(170,7,utf8_decode('Atención de Emergencias 911., en un horario de 24 horas  por 72 horas libres.'),0,1);
		}
